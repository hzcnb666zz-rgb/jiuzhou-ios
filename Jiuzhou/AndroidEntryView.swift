        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                VStack(alignment: .center, spacing: 6) {
                    ...
                    Group {
                        if let path = Bundle.main.path(forResource: "charcreate_title", ofType: "png"),
                           let image = UIImage(contentsOfFile: path) {
                            ...
                            Text("创建你的角色").font(.system(size: 19, weight: .bold))
                    ...
                    Text("你的称呼，2-4个中文字符")
                        .font(.system(size: 11)).foregroundStyle(Color(red: 228/255, green: 195/255, blue: 125/255))
                        .frame(maxWidth: .infinity)
                    ...
                    Text("你的性别").font(.system(size: 11)).foregroundStyle(Color(red: 228/255, green: 195/255, blue: 125/255))
                        .frame(maxWidth: .infinity)
                    HStack(spacing: 12) {
                        genderButton("男", "男性")
                        genderButton("女", "女性")
                    ...
                    // 创建（左）/取消（右），收在卡片底部
                    HStack(spacing: 10) {
                        martialButton("创 建", glow: true, height: 28) { game.createCharacter(name: characterName, gender: gender) }
                        martialButton("取 消", glow: false, height: 28) { game.logout() }
                    }

                    Text(game.notice).font(.system(size: 11)).foregroundStyle(Color(red: 240/255, green: 215/255, blue: 160/255))
                        .frame(maxWidth: .infinity)
                }
                .padding(10)

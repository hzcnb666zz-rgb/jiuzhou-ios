...
struct MudStyleStream {
    ...
    mutating func render(_ text: String) -> String {
        let prefix = ["bold", "fullwidth", "foreground", "brightForeground", "background", "normalBackground", "size", "link"].compactMap { spans[$0] }.joined()
        ...
        for match in Self.tags.matches(in: text, range: NSRange(location: 0, length: ns.length)) {
            ...
            if code.hasPrefix("u:") { spans["link"] = tag }
                ...
                if numbers.contains(where: { (30...37).contains($0) }) {
                    spans[numbers.contains(1) ? "brightForeground" : "foreground"] = tag
                }
                if numbers.contains(where: { (40...47).contains($0) }) {
                    spans[numbers.contains(1) ? "background" : "normalBackground"] = tag
                }
            }
        }
        return prefix + text

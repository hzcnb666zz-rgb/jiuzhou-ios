...
## 后续修正说明（构建 12，验收结果见 ANDROID-PARITY-AUDIT.md）

本文件下方保留构建 11 的历史问题，不代表当前代码仍全部未修复。
后续已修正重复动作/对象身份、标题动作与特殊出口尺寸、方向替换、
属性条末行权重、跨消息富文本状态，并增加注册、会员中心、语音和战斗浮字实现。

F03 中“缺少邮箱”的表述需要纠正：`loginx.xml` 的邮箱父布局是 GONE，
控件预置 `a1@qq.com`。安卓运行界面只有账号、密码、确认密码、手机号四项。
`ucenterx.xml` 的邮箱行同样为 GONE。iOS 应保留隐藏字段的请求值，不能增加可见行。
此前依据 Java 字段校验推断可见 UI 是错误的；本轮已用 XML 父级可见性和 APK 实际运行交叉确认。

语音编解码、请求格式和入口测试与真实服务发送成功是不同证据；
真实 TCP 测试使用本地测试服务，也不等于用户真机已成功登录当前游戏服务器。
最终包、测试数量及仍未实测的项目统一记录在主审计文档中。
